...
impl CursorRenderer {
    ...
    pub fn update_cursor_state(&mut self, cur: CursorPosition, text_width: f32, text_height: f32) {
        ...
        let cursor_rect = match &self.cursor_type {
            &CursorType::Block(_)
            | &CursorType::SolidBlock(_)
            | &CursorType::OpenBlock(_)
            | &CursorType::None => Rect::from_min_size(
                egui::pos2(
                    (cur.x) as f32 * text_width + self.widget_offset.x + 0.5,
            ...
            &CursorType::Beam(_) => Rect::from_min_size(
                ...
                egui::vec2(text_width - 6., text_height),
            ),
        };
    ...
    fn get_trail_color(&self) -> Color32 {
        self.trail_color
            .unwrap_or_else(|| self.get_color().gamma_multiply(0.5))
    }

    fn get_color(&self) -> Color32 {
        let (alpha, color) = match self.cursor_type {
            CursorType::Block(c) => (
                ((self.stable_time_factor / std::f64::consts::FRAC_PI_2 * 13.).sin() + 1.) / 2.,
                c,
            ),
    ...
    pub fn draw_cursor(&mut self, painter: Painter, delta_time: f32) {
        ...
        if matches!(self.cursor_type, CursorType::None) {
            return;
        }
        if !self.visible {
            return;
