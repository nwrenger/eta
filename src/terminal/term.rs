...
use super::config::definitions::TermResult;
use super::config::term_config::{Config, Style};
use super::into::*;
use super::render::CursorRenderer;
...
impl TermHandler {
    ...
    fn generate_rows(&mut self, ui: &mut Ui) -> Response {
        ...
        let total_height = self.terminal.screen().scrollback_rows() as f32;
        let offset =
            (self.terminal.screen().scrollback_rows() - size.rows) as f32 - ui.clip_rect().min.y;

        let mut job = egui::text::LayoutJob::default();
        let first_visible_row = (offset).floor() as usize;
        let last_visible_row = (offset + total_height).ceil() as usize;
        ...
        let mut iter = self
            .terminal
            .screen()
            .lines_in_phys_range(visible_range)
            .into_iter()
            .peekable();
    ...
    pub(crate) fn draw(
        ...
        if self.was_focused {
            self.cursor_renderer.cursor_type = self.style.default_focus_cursor
        } else {
        ...
        ui.memory_mut(|mem| mem.set_focus_lock_filter(r.id, event_filter));
